...
function Navbar() {
  // theme comes from localStorage, default light
  const [theme, setTheme] = useState(localStorage.getItem("theme") || "light");
  const el = document.documentElement;

  // apply html.dark class when theme === "dark"
  useEffect(() => {
    if (theme === "dark") {
  ...
  // top-level container classes depend on sticky + theme
  const containerClass = `fixed inset-x-0 top-0 z-50 transition-colors duration-300 ${
    sticky
      ? theme === "dark"
    ...
    <header className={containerClass}>
      <div className="max-w-screen-2xl mx-auto md:px-20 px-4">
        <nav className="flex items-center justify-between py-3">
          {/* left: logo + mobile dropdown placeholder */}
          <div className="flex items-center gap-3">
            <div className="lg:hidden">
              <button
                aria-label="menu"
                className={`p-2 rounded-md ${theme === "dark" ? "text-white" : "text-black"}`}
              >
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="hidden md:flex items-center border rounded-md px-2 py-1 bg-transparent">
              <svg className="h-5 w-5 opacity-60" viewBox="0 0 24 24" fill="none" stroke="currentColor">
            ...
            {/* theme toggle: original icons, h-8 w-8, visible in both modes */}
            <button
            ...
            >
              {theme === "light" ? (
                // Sun icon (same size/shape you used)
                <svg
              ...
              >
                ...
                // Moon icon (same size/shape you used)
                <svg
            ...
            {/* login */}
            <a className={`px-3 py-2 rounded-md ${theme === "dark" ? "bg-white text-black" : "bg-black text-white"}`}
...
//     <>
//       <div
//         className={`max-w-screen-2xl container mx-auto md:px-20 px-4 dark:bg-slate-900 dark:text-white fixed top-0 right-0 z-50 ${
//           sticky
...
//                 Login
//               </a>{" "}
//               {/* button next to About */}
//             </div>

//             {/* Navbar end (for mobile, optional) */}
//             <div className="navbar-end lg:hidden">
//               <a className="bg-black text-white">Login</a>
//             </div>
//           </div>
//         </div>
//       </div>
